...
2. Decompose A into lower and upper triangular matrices so that A = LU.
   In Crout's method, every diagonal element of U is 1.
...
int main() {
    ...
    /* Crout decomposition: calculate one column at a time. */
    for (j = 0; j < n; j++) {
        /* Calculate column j of the lower-triangular matrix L. */
        for (i = j; i < n; i++) {
            sum = 0.0;
            for (k = 0; k < j; k++) {
        ...
        if (absoluteValue(lower[j][j]) < EPSILON) {
            printf("Crout decomposition failed because a pivot is zero.\n");
            return 1;
        }

        upper[j][j] = 1.0;

        /* Calculate row j of the upper-triangular matrix U. */
        for (i = j + 1; i < n; i++) {
            sum = 0.0;
            for (k = 0; k < j; k++) {
